...
/{BotCommands.MirrorCommand[0]} link

<b>By replying to link/file</b>:

/{BotCommands.MirrorCommand[0]} -n new name -e -up upload destination
...
/{BotCommands.YtdlCommand[0]} link
<b>By replying to link</b>:
/{BotCommands.YtdlCommand[0]} -n new name -z password -opt x:y|x1:y1
...
Use -sync to use sync method in rclone. Example: /{BotCommands.CloneCommand} rcl/rclone_path -up rcl/rclone_path/rc -sync"""

new_name = f"""<b>New Name</b>: -n

/{BotCommands.MirrorCommand[0]} link -n new name
Note: Doesn't work with torrents"""

multi_link = f"""<b>Multi links only by replying to first link/file</b>: -i

/{BotCommands.MirrorCommand[0]} -i 10(number of links/files)"""

same_dir = f"""<b>Move file(s)/folder(s) to new folder</b>: -m
...
/{BotCommands.MirrorCommand[0]} link -m new folder (only one link inside new folder)
/{BotCommands.MirrorCommand[0]} -i 10(number of links/files) -m folder name (all links contents in one folder)
/{BotCommands.MirrorCommand[0]} -b -m folder name (reply to batch of message/file(each link on new line))
...
thumb = f"""<b>Thumbnail for current task</b>: -t

/{BotCommands.MirrorCommand[0]} link -t tg-message-link (doc or photo) or none (file without thumb)"""

split_size = f"""<b>Split size for current task</b>: -sp

/{BotCommands.MirrorCommand[0]} link -sp (500mb or 2gb or 4000000000)
Note: Only mb and gb are supported or write in bytes without unit!"""

upload = f"""<b>Upload Destination</b>: -up

/{BotCommands.MirrorCommand[0]} link -up rcl/gdl (rcl: to select rclone config, remote & path | gdl: To select token.pickle, gdrive id) using buttons
...
/{BotCommands.MirrorCommand[0]} link -up mrcc:main:dump or -up mtp:gdrive_id <strong>or you can simply edit upload using owner/user token/config from usetting without adding mtp: or mrcc: before the upload path/id</strong>
...
user_download = f"""<b>User Download</b>: link

/{BotCommands.MirrorCommand[0]} tp:link to download using owner token.pickle incase service account enabled.
/{BotCommands.MirrorCommand[0]} sa:link to download using service account incase service account disabled.
/{BotCommands.MirrorCommand[0]} tp:gdrive_id to download using token.pickle and file_id incase service account enabled.
/{BotCommands.MirrorCommand[0]} sa:gdrive_id to download using service account and file_id incase service account disabled.
/{BotCommands.MirrorCommand[0]} mtp:gdrive_id or mtp:link to download using user token.pickle uploaded from usetting
/{BotCommands.MirrorCommand[0]} mrcc:remote:path to download using user rclone config uploaded from usetting
you can simply edit upload using owner/user token/config from usetting without adding mtp: or mrcc: before the path/id"""

rcf = f"""<b>Rclone Flags</b>: -rcf

/{BotCommands.MirrorCommand[0]} link|path|rcl -up path|rcl -rcf --buffer-size:8M|--drive-starred-only|key|key:value
This will override all other flags except --exclude
Check here all <a href='https://rclone.org/flags/'>RcloneFlags</a>."""

bulk = f"""<b>Bulk Download</b>: -b
...
Reply to this example by this cmd -> /{BotCommands.MirrorCommand[0]} -b(bulk)

Note: Any arg along with the cmd will be setted to all links
/{BotCommands.MirrorCommand[0]} -b -up remote: -z -m folder name (all links contents in one zipped folder uploaded to one destination)
...
rlone_dl = f"""<b>Rclone Download</b>:

Treat rclone paths exactly like links
/{BotCommands.MirrorCommand[0]} main:dump/ubuntu.iso or rcl(To select config, remote and path)
Users can add their own rclone from user settings
If you want to add path manually from your config add mrcc: before the path without space
/{BotCommands.MirrorCommand[0]} mrcc:main:dump/ubuntu.iso
You can simply edit using owner/user config from usetting without adding mrcc: before the path"""

extract_zip = f"""<b>Extract/Zip</b>: -e -z

/{BotCommands.MirrorCommand[0]} link -e password (extract password protected)
/{BotCommands.MirrorCommand[0]} link -z password (zip password protected)
/{BotCommands.MirrorCommand[0]} link -z password -e (extract and zip password protected)
Note: When both extract and zip added with cmd it will extract first and then zip, so always extract first"""

join = f"""<b>Join Splitted Files</b>: -j

This option will only work before extract and zip, so mostly it will be used with -m argument (samedir)
By Reply:
/{BotCommands.MirrorCommand[0]} -i 3 -j -m folder name
/{BotCommands.MirrorCommand[0]} -b -j -m folder name
if u have link(folder) have splitted files:
/{BotCommands.MirrorCommand[0]} link -j"""
...
sample_video = f"""<b>Sample Video</b>: -sv

Create sample video for one video or folder of videos.
/{BotCommands.MirrorCommand[0]} -sv (it will take the default values which 60sec sample duration and part duration is 4sec).
You can control those values. Example: /{BotCommands.MirrorCommand[0]} -sv 70:5(sample-duration:part-duration) or /{BotCommands.MirrorCommand[0]} -sv :5 or /{BotCommands.MirrorCommand[0]} -sv 70."""

screenshot = f"""<b>ScreenShots</b>: -ss

Create screenshots for one video or folder of videos.
/{BotCommands.MirrorCommand[0]} -ss (it will take the default values which is 10 photos).
You can control this value. Example: /{BotCommands.MirrorCommand[0]} -ss 6."""

seed = f"""<b>Bittorrent seed</b>: -d

/{BotCommands.MirrorCommand[0]} link -d ratio:seed_time or by replying to file/link
...
zip_arg = f"""<b>Zip</b>: -z password

/{BotCommands.MirrorCommand[0]} link -z (zip)
/{BotCommands.MirrorCommand[0]} link -z password (zip password protected)"""

qual = f"""<b>Quality Buttons</b>: -s
...
/{BotCommands.YtdlCommand[0]} link -s"""

yt_opt = f"""<b>Options</b>: -opt

/{BotCommands.YtdlCommand[0]} link -opt {{"format": "bv*+mergeall[vcodec=none]", "nocheckcertificate": True, "playliststart": 10, "fragment_retries": float("inf"), "matchtitle": "S13", "writesubtitles": True, "live_from_start": True, "postprocessor_args": {{"ffmpeg": ["-threads", "4"]}}, "wait_for_video": (5, 100), "download_ranges": [{{"start_time": 0, "end_time": 10}}]}}

Check all yt-dlp api options from this <a href='https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/YoutubeDL.py#L184'>FILE</a> or use this <a href='https://t.me/mltb_official_channel/177'>script</a> to convert cli arguments to api options."""

convert_media = f"""<b>Convert Media</b>: -ca -cv
/{BotCommands.MirrorCommand[0]} link -ca mp3 -cv mp4 (convert all audios to mp3 and all videos to mp4)
/{BotCommands.MirrorCommand[0]} link -ca mp3 (convert all audios to mp3)
/{BotCommands.MirrorCommand[0]} link -cv mp4 (convert all videos to mp4)
/{BotCommands.MirrorCommand[0]} link -ca mp3 + flac ogg (convert only flac and ogg audios to mp3)
/{BotCommands.MirrorCommand[0]} link -cv mkv - webm flv (convert all videos to mp4 except webm and flv)"""

force_start = f"""<b>Force Start</b>: -f -fd -fu
/{BotCommands.MirrorCommand[0]} link -f (force download and upload)
/{BotCommands.MirrorCommand[0]} link -fd (force download only)
/{BotCommands.MirrorCommand[0]} link -fu (force upload directly after download finish)"""

gdrive = f"""<b>Gdrive</b>: link
If DEFAULT_UPLOAD is `rc` then you can pass up: `gd` to upload using gdrive tools to GDRIVE_ID.
/{BotCommands.MirrorCommand[0]} gdriveLink or gdl or gdriveId -up gdl or gdriveId or gd
/{BotCommands.MirrorCommand[0]} tp:gdriveLink or tp:gdriveId -up tp:gdriveId or gdl or gd (to use token.pickle if service account enabled)
/{BotCommands.MirrorCommand[0]} sa:gdriveLink or sa:gdriveId -p sa:gdriveId or gdl or gd (to use service account if service account disabled)
/{BotCommands.MirrorCommand[0]} mtp:gdriveLink or mtp:gdriveId -up mtp:gdriveId or gdl or gd(if you have added upload gdriveId from usetting) (to use user token.pickle that uploaded by usetting)
You can simply edit using owner/user token from usetting without adding mtp: before the id"""

rclone_cl = f"""<b>Rclone</b>: path
If DEFAULT_UPLOAD is `gd` then you can pass up: `rc` to upload to RCLONE_PATH.
/{BotCommands.MirrorCommand[0]} rcl/rclone_path -up rcl/rclone_path/rc -rcf flagkey:flagvalue|flagkey|flagkey:flagvalue
/{BotCommands.MirrorCommand[0]} rcl or rclone_path -up rclone_path or rc or rcl
/{BotCommands.MirrorCommand[0]} mrcc:rclone_path -up rcl or rc(if you have add rclone path from usetting) (to use user config)
You can simply edit using owner/user config from usetting without adding mrcc: before the path"""

name_sub = r"""<b>Name Substitution</b>: -ns
/{BotCommands.MirrorCommand[0]} link -ns script/code/s | mirror/leech | tea/ /s | clone | cpu/ | \[mltb\]/mltb | \\text\\/text/s
...
transmission = f"""<b>Tg transmission</b>: -hl -ut -bt
/{BotCommands.MirrorCommand[0]} link -hl (leech by user and bot session with respect to size) (Hybrid Leech)
/{BotCommands.MirrorCommand[0]} link -bt (leech by bot session)
/{BotCommands.MirrorCommand[0]} link -ut (leech by user)"""

thumbnail_layout = f"""Thumbnail Layout: -tl
/{BotCommands.MirrorCommand[0]} link -tl 3x3 (widthxheight) 3 photos in row and 3 photos in column"""

leech_as = f"""<b>Leech as</b>: -doc -med
/{BotCommands.LeechCommand[0]} link -doc (Leech as document)
/{BotCommands.LeechCommand[0]} link -med (Leech as media)"""
...
MIRROR_HELP_DICT = {
    "main": mirror,
    "New-Name": new_name,
    "DL-Auth": "<b>Direct link authorization</b>: -au -ap\n\n/{BotCommands.MirrorCommand[0]} link -au username -ap password",
    "Headers": "<b>Direct link custom headers</b>: -h\n\n/{BotCommands.MirrorCommand[0]} link -h key:value|key1:value1",
    "Extract/Zip": extract_zip,
    "Select-Files": "<b>Bittorrent/JDownloader/Sabnzbd File Selection</b>: -s\n\n/{BotCommands.MirrorCommand[0]} link -s or by replying to file/link",
    "Torrent-Seed": seed,
    "Multi-Link": multi_link,
    "Same-Directory": same_dir,
    "Thumb": thumb,
...
Title2 link -c /{BotCommands.MirrorCommand[0]} -inf xx -exf xx
Title3 link -c /{BotCommands.LeechCommand[0]} -d ratio:time -z password
...
user_settings_text = {
    "LEECH_SPLIT_SIZE": f"Send Leech split size in bytes or use gb or mb. Example: 40000000 or 2.5gb or 1000mb. IS_PREMIUM_USER: {g_context.IS_PREMIUM_USER}. Timeout: 60 sec",
