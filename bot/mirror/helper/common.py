...
class TaskConfig:
    ...
    async def before_start(self):
        ...
        self.user_transmission = g_context.IS_PREMIUM_USER and (
        ...
        if not self.is_leech:
            ...
            self.hybrid_leech = g_context.IS_PREMIUM_USER and (
            ...
            if self.bot_trans:
                ...
                self.hybrid_leech = False
            if self.user_trans:
                self.user_transmission = g_context.IS_PREMIUM_USER
            if self.up_dest:
                ...
                if self.user_transmission:
                    try:
                        chat = await g_context.user.get_chat(self.up_dest)
                    except:
                        chat = None
                    if chat is None or not g_context.user:
                        self.user_transmission = False
                        self.hybrid_leech = False
                    else:
                        uploader_id = g_context.user.me.id
            ...
            self.max_split_size = (
                g_context.MAX_SPLIT_SIZE if self.user_transmission else 2097152000
            )
