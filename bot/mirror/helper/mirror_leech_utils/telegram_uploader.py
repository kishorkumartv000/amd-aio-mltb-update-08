...
class TelegramUploader:
    ...
    async def _upload_progress(self, current, _):
        if self._listener.is_cancelled:
            if self._user_session:
                if g_context.user:
                    g_context.user.stop_transmission()
            else:
    ...
    async def _msg_to_reply(self):
        if self._listener.up_dest:
            ...
            try:
                if self._user_session and g_context.user:
                    self._sent_msg = await g_context.user.send_message(
                ...
                return False
        elif self._user_session and g_context.user:
            self._sent_msg = await g_context.user.get_messages(
    ...
    async def _send_media_group(self, subkey, key, msgs):
        for index, msg in enumerate(msgs):
            if self._listener.hybrid_leech or not self._user_session or not g_context.user:
                msgs[index] = await self._listener.client.get_messages(
                    chat_id=msg[0], message_ids=msg[1]
                )
            else:
                msgs[index] = await g_context.user.get_messages(
                    chat_id=msg[0], message_ids=msg[1]
                )
