...
class TelegramDownloadHelper:
    ...
    async def _on_download_progress(self, current, _):
        if self._listener.is_cancelled:
            if self.session == "user":
                ...
                if g_context.bot:
                    g_context.bot.stop_transmission()
    ...
    async def add_download(self, message, path, session):
        ...
        if not self.session:
            if self._listener.user_transmission and self._listener.is_super_chat and g_context.user:
                self.session = "user"
                message = await g_context.user.get_messages(
