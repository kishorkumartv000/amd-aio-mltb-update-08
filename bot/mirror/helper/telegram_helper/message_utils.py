...
async def send_rss(text, chat_id, thread_id):
    try:
        app = g_context.user or g_context.bot
        if not app:
            LOGGER.error("No client found to send RSS message.")
            return
...
async def get_tg_link_message(link):
    ...
    if not private:
        try:
            message = await g_context.bot.get_messages(chat_id=chat, message_ids=msg_id)
            if message.empty:
                private = True
        except Exception as e:
            private = True
    ...
    if not private:
        ...
        try:
            user_message = await g_context.user.get_messages(
