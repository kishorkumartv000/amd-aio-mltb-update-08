...
class TaskListener(TaskConfig):
    ...
    async def on_local_upload(self, path, gid):
        """
        Called from a callback to start an upload for a local path.
        Bypasses the download and argument parsing steps.
        """
        self.up_dir = path
        self.name = path.rsplit("/", 1)[-1]
        self.size = await get_path_size(self.up_dir)
        self.is_file = await aiopath.isfile(self.up_dir)

        LOGGER.info(f"Starting local upload for: {self.name}")

        # Use default upload destination
        if not self.is_leech:
            self.up_dest = Config.MIRROR_DEFAULT_UPLOAD
            if self.up_dest == 'rc':
                self.up_dest = Config.MIRROR_RCLONE_PATH
            else:
                self.up_dest = Config.MIRROR_GDRIVE_ID

        if not self.up_dest:
            await self.on_upload_error("No upload destination configured!")
            return
        ...
        if self.is_leech:
            LOGGER.info(f"Leech Name: {self.name}")
            tg = TelegramUploader(self, self.up_dir)
            async with task_dict_lock:
                task_dict[self.mid] = TelegramStatus(self, tg, gid, "up")
            await gather(update_status_message(self.message.chat.id), tg.upload())
        elif is_gdrive_id(self.up_dest):
            LOGGER.info(f"Gdrive Upload Name: {self.name}")
            drive = GoogleDriveUpload(self, self.up_dir)
            async with task_dict_lock:
                task_dict[self.mid] = GoogleDriveStatus(self, drive, gid, "up")
            ...
            async with task_dict_lock:
                task_dict[self.mid] = RcloneStatus(self, RCTransfer, gid, "up")
            await gather(update_status_message(self.message.chat.id), RCTransfer.upload(self.up_dir))
