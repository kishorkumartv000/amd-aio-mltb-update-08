...
async def send_incomplete_task_message(cid, msg_id, msg):
    if not g_context.bot:
        return
    try:
        if msg.startswith("Restarted Successfully!"):
            await g_context.bot.edit_message_text(
                chat_id=cid,
                message_id=msg_id,
                text=msg,
            ...
            await g_context.bot.send_message(
                chat_id=cid,
                text=msg,
                disable_web_page_preview=True,
...
async def confirm_restart(_, query):
    ...
    if data[1] == "confirm":
        ...
        if g_context.bot:
            await g_context.bot.stop()
        if g_context.user:
            await g_context.user.stop()
        ...
        proc1 = await create_subprocess_exec(
            ...
            "-9",
            "-f",
            "gunicorn|aria2c|qbittorrent-nox|ffmpeg|rclone|sabnzbdplus|7z|split",
        )
