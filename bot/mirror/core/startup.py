...
async def save_settings():
    ...
    if g_context.bot:
        await database.db.settings.config.replace_one(
            {"_id": g_context.bot.me.id}, config_dict, upsert=True
        )


async def update_variables():
    if (
        Config.LEECH_SPLIT_SIZE > g_context.MAX_SPLIT_SIZE
        or Config.LEECH_SPLIT_SIZE == 2097152000
        or not Config.LEECH_SPLIT_SIZE
    ):
        Config.LEECH_SPLIT_SIZE = g_context.MAX_SPLIT_SIZE

    Config.HYBRID_LEECH = bool(Config.HYBRID_LEECH and g_context.IS_PREMIUM_USER)
    Config.USER_TRANSMISSION = bool(
        Config.USER_TRANSMISSION and g_context.IS_PREMIUM_USER
    )
